...
import { uploadImage, fetchDetectionLogs, logLiveEvent, fetchStats } from "../../api";
...
export const Dashboard = () => {
  ...
  // Fetch real-time security score from backend
  useEffect(() => {
    const fetchSecurityScore = async () => {
      try {
        const stats = await fetchStats();
        if (stats?.security_score !== undefined) {
          setSecurityScore(stats.security_score);
        }
      } catch (error) {
    ...
    const scoreInterval = setInterval(fetchSecurityScore, 3000);
